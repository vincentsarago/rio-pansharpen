...
def test_data():
    ...
    test_data_rgb = np.array(
        [(np.random.rand(30, 30) * 255).astype(np.uint8) for i in range(3)]
    )
...
def test_pansharp_data():
    ...
    g_args = {
        "half_window": False,
        "dst_aff": Affine(
            75.00483870967741, 0.0, 300892.5, 0.0, -75.00475285171103, 4107007.5
        ),
        "verb": False,
        "weight": 0.2,
        ...
        "r_aff": Affine(
            150.0193548387097, 0.0, 300885.0, 0.0, -150.0190114068441, 4107015.0
        ),
        "src_nodata": 0,
    }
...
def test_reproject():
    ...
    with rasterio.Env():
        ...
        dst_shape = (2048, 2048)
        dst_transform = Affine.from_gdal(-237481.5, 425.0, 0.0, 237536.4, 0.0, -425.0)
...
def test_upsample(test_data):
    ...
    with rasterio.Env():
        ...
        up_rgb = utils._upsample(rgb, pan.shape, src_aff, src_crs, dst_aff, dst_crs)
...
def test_brovey(test_data):
    pan, rgb, src_aff, src_crs, dst_aff, dst_crs = test_data
    up_rgb = np.array(
        [(np.random.rand(60, 60) * 255).astype(np.uint8) for i in range(3)]
    )
