...
from rio_pansharpen.methods import calculateRatio, Brovey
...
def test_simple_mask(data, ndv):
    ...
    assert np.array_equal(
        ...
        np.invert(np.all(np.dstack(data) == ndv, axis=2)).astype(data.dtype) * nd,
    )
...
@given(
    st.tuples(
        st.tuples(st.integers(), st.integers()), st.tuples(st.integers(), st.integers())
    ),
    st.integers(),
)
        ...
        assert np.array_equal(
            ...
            np.concatenate(
                [
                    ...
                    _simple_mask(arr.astype(dst_dtype), (ndv, ndv, ndv)).reshape(
...
def test_make_windows_full_block(w, h, blocksize):
    ...
    if wind < 2:
        assert (
            windows[wind][0][1] - windows[wind][0][0] <= blocksize
...
def test_make_windows_last_block(w, h, blocksize):
    ...
    assert (
        windows[-1][0][1] - windows[-1][0][0] <= blocksize
...
@given(
    st.tuples(
        st.tuples(st.integers(min_value=2), st.integers(min_value=2)).filter(
            lambda x: x[0] < x[1]
        ),
        st.tuples(st.integers(min_value=2), st.integers(min_value=2)).filter(
            lambda x: x[0] < x[1]
