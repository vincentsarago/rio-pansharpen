...
def pansharpen(
    ...
    """
    rgb = _upsample(
        _create_apply_mask(vis), pan.shape, vis_transform, r_crs, pan_transform, dst_crs
    )
...
def calculate_landsat_pansharpen(
    ...
    if profile["width"] <= r_meta["width"] or profile["height"] <= r_meta["height"]:
