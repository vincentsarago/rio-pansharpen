"""Rio-pansharpen Utility functions."""
...
def _make_windows(width, height, blocksize):
    """Manually makes windows of size equivalent to pan band image."""
    for x in range(0, width, blocksize):
        for y in range(0, height, blocksize):
            yield Window(
                y, min((y + blocksize), height), x, min((x + blocksize), width)
            )


def _make_affine(fr_shape, to_shape):
    """Given from and to width and height, compute affine transform defining the
    georeferencing of the output array.
    """
    fr_window_affine = Affine(1, 0, 0, 0, -1, 0)

    to_window_affine = Affine(
        (fr_shape[1] / float(to_shape[1])),
        0,
        0,
        0,
        (fr_shape[0] / float(to_shape[0])),
        0,
    )
    ...
    for i in range(1, len(inputs)):
        if inputs[i - 1]["crs"] != inputs[i]["crs"]:
            raise RuntimeError(
                ...
                "received %s and %s" % (inputs[i - 1]["crs"], inputs[i]["crs"])
            )
...
def _create_apply_mask(rgb):
    ...
    """
    color_mask = (
        np.all(np.rollaxis(rgb, 0, 3) != 0, axis=2).astype(np.uint16)
...
def _calc_windows(pan_src, customwindow):
    ...
    if customwindow != 0 and isinstance(customwindow, int):
        blocksize = _adjust_block_size(
            pan_src.meta["width"], pan_src.meta["height"], int(customwindow)
        )
        windows = [
            (window, (0, 0))
            for window in _make_windows(
                pan_src.meta["width"], pan_src.meta["height"], blocksize
            )
        ]
    else:
        windows = [(window.toranges(), ij) for ij, window in pan_src.block_windows()]
...
def _rescale(arr, ndv, dst_dtype, out_alpha=True):
    ...
    if out_alpha:
        mask = _simple_mask(arr.astype(dst_dtype), (ndv, ndv, ndv)).reshape(
